...
window.fetch = async (...args) => {
    ...
    if (response.status === 403) {
        window.location.href = '/?msg=Access+Denied:+Insufficient+permissions+for+Value+Tracker.';
        return response;
    }
    return response;
};
...
const FREE_TEXT_COLS = ["Comments","ETA"]
...
const GREEN_VALUES = ["Shared","Completed"]
...
async function selectProject(name, el) {
    ...
    const res = await fetch(`/value/api/projects/${encodeURIComponent(name)}`)
    const rows = await res.json()
    const alertsProject=await fetch(`/value/api/alerts/${encodeURIComponent(name)}`)
    const alerts=await alertsProject.json()
    const dueRes=await fetch(`/value/api/due-today/${encodeURIComponent(name)}`)
...
async function openPanel(row) {
    ...
    const [statusRes, deadlineRes,etaVal] = await Promise.all([
        fetch(`/value/api/status/${row.id}`),
        fetch(`/value/api/deadlines/${row.id}`),
    ...
    STATUS_COLS.forEach(col => {
        ...
        if (col==="ETA"){
            html += `<div class="field-group">
                ...
                <input type="date" id="status_${col}" value="${etaRes["eta"]}" 
...
async function savePanel() {
    ...
    STATUS_COLS.forEach(col => {
        if (col === "ETA") return  // handle separately
    ...
    const etaEl = document.getElementById("status_ETA")
    const etaPromise = etaEl && etaEl.value
        ? fetch(`/value/api/projects/${currentRowId}/eta`, {
            method: "PUT",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ eta: etaEl.value })
          })
        : Promise.resolve()

    await Promise.all([
        fetch(`/value/api/status/${currentRowId}`, {
            method: "PUT",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify(statusUpdates)
        }),
        fetch(`/value/api/deadlines/${currentRowId}`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
...
async function loadAlerts() {
    const res = await fetch("/value/api/alerts")
...
async function openPanelById(projectId) {
    const res = await fetch(`/value/api/projects/id/${projectId}`)
...
async function addProject(){
    ...
    const etaInput = document.getElementById("new-project-eta");
    const vendorInput=document.getElementById("new-project-vendor");
    const pmCodeInput=document.getElementById("new-project-pm-code");
    const today = new Date().toISOString().split("T")[0]
    ...
    const pmCode=pmCodeInput.value

    if(!name || !packagingType || !packagingOption || !eta || !vendor || !pmCode){
        alert("Please fill all the fields")
        return
    }

    const res = await fetch("/value/api/projects", { // Added leading absolute slash
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
            Project: name,
...
async function deleteProject(){
    ...
    const res = await fetch(`/value/api/projects/${currentRowId}`, { 
        method: "DELETE" 
    })
    ...
    if(res.ok){
        ...
        if (activeProject) {
            const refreshRes = await fetch(`/value/api/projects/${encodeURIComponent(activeProject.dataset.fullname)}`)
...
async function downloadExcel(){
    ...
    const res = await fetch(`/value/api/download/${encodeURIComponent(fileName)}`);
...
async function loadDueToday() {
    const res = await fetch("/value/api/due-today")
